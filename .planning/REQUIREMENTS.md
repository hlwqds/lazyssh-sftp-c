...
- [ ] **DEL-01**: 用户在任一面板选中文件后按 `d` 键，弹出确认对话框显示待删除文件名和大小，用户确认后执行删除
- [ ] **DEL-02**: 用户删除目录时系统递归删除目录内所有文件和子目录，删除过程中显示进度
- [ ] **DEL-03**: 用户通过 Space 多选文件后按 `d` 键，确认对话框显示待删除文件数量和总大小，确认后批量删除
- [ ] **DEL-04**: 删除操作完成后自动刷新当前目录列表，光标定位到合理位置
- [ ] **REN-01**: 用户在任一面板选中文件/目录后按 `R` 键，弹出输入框预填当前文件名，用户编辑后按 Enter 完成重命名，按 Esc 取消
- [ ] **REN-02**: 重命名时如果目标名称已存在，提示名称冲突，用户可选择覆盖或取消
- [ ] **MKD-01**: 用户在任一面板按 `m` 键，弹出输入框输入目录名，按 Enter 在当前目录下创建子目录，按 Esc 取消
- [ ] **MKD-02**: 新建目录完成后自动刷新列表，光标定位到新创建的目录

## P2: 复制与移动

- [ ] **CPY-01**: 用户选中文件/目录后按 `c` 键，文件被标记为复制源（剪贴板状态），文件列表中显示 `[C]` 前缀标记
- [ ] **CPY-02**: 用户导航到目标目录后按 `p` 键，系统将标记的文件/目录复制到当前目录
- [ ] **CPY-03**: 复制目录时递归复制所有内容，远程端通过 download+re-upload 实现
...
- [ ] **CLP-01**: 剪贴板有标记时，被标记文件在列表中显示 `[C]`（复制）或 `[M]`（移动）前缀
- [ ] **CLP-02**: 用户导航到其他目录后，剪贴板标记仍然保留（跨目录复制/移动）
- [ ] **CLP-03**: 用户按 `Esc` 或新的 `c`/`x` 操作时清除之前剪贴板标记
- [ ] **RCP-01**: 远程复制（download+re-upload）显示统一进度视图，包含已复制文件数和总大小
