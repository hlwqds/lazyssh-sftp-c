...
- [ ] **HIST-01**: 用户在远程面板中导航到新目录时（Enter 进入子目录、h 返回上级），系统自动将该目录路径记录到最近目录列表
- [ ] **HIST-02**: 最近目录列表按 MRU（Most Recently Used）排序，最近访问的路径排在最前
- [ ] **HIST-03**: 同一目录路径多次访问时自动去重，仅保留最新位置（移到列表头部）
- [ ] **HIST-04**: 最近目录列表最多保留 10 条记录，超出时移除最旧的条目
