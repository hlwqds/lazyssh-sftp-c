...
- [x] **MOV-01**: 用户选中文件/目录后按 `x` 键，文件被标记为移动源（剪贴板状态），文件列表中显示 `[M]` 前缀标记
- [x] **MOV-02**: 用户导航到目标目录后按 `p` 键，系统将标记的文件/目录移动到当前目录（复制+删除源文件）
- [x] **MOV-03**: 移动操作失败时（如权限不足），不删除源文件，保留原始状态
- [x] **PRG-01**: 复制/移动大文件或目录时显示进度条（复用 TransferModal 或状态栏进度显示）
- [x] **CNF-01**: 复制到目标目录时如果同名文件已存在，弹出冲突对话框（覆盖/跳过/重命名）
- [x] **CNF-02**: 多文件复制/移动时，每个冲突文件单独询问处理方式
